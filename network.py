...
class BaseNetwork(nn.Module):
    ...
    def print_network(self):
        ...
        print("Network [{}] was created. Total number of parameters: {:.1f} million. "
              "To see the architecture, do print(network).".format(self._class.name_, num_params / 1000000))
...
class SegGenerator(BaseNetwork):
    def _init_(self, opt, input_nc, output_nc=13, norm_layer=nn.InstanceNorm2d):
        super(SegGenerator, self)._init_()
...
class FeatureExtraction(BaseNetwork):
    def _init_(self, input_nc, ngf=64, num_layers=4, norm_layer=nn.BatchNorm2d):
        super(FeatureExtraction, self)._init_()
...
class FeatureCorrelation(nn.Module):
    def _init_(self):
        super(FeatureCorrelation, self)._init_()
...
class FeatureRegression(nn.Module):
    def _init_(self, input_nc=512, output_size=6, norm_layer=nn.BatchNorm2d):
        super(FeatureRegression, self)._init_()
...
class TpsGridGen(nn.Module):
    def _init_(self, opt, dtype=torch.float):
        super(TpsGridGen, self)._init_()
...
class GMM(nn.Module):
    def _init_(self, opt, inputA_nc, inputB_nc):
        super(GMM, self)._init_()
...
class MaskNorm(nn.Module):
    def _init_(self, norm_nc):
        super(MaskNorm, self)._init_()
...
class ALIASNorm(nn.Module):
    def _init_(self, norm_type, norm_nc, label_nc):
        super(ALIASNorm, self)._init_()
...
class ALIASResBlock(nn.Module):
    def _init_(self, opt, input_nc, output_nc, use_mask_norm=True):
        super(ALIASResBlock, self)._init_()
...
class ALIASGenerator(BaseNetwork):
    def _init_(self, opt, input_nc):
        super(ALIASGenerator, self)._init_()
    ...
    def forward(self, x, seg, seg_div, misalign_mask):
        ...
        features = [self.modules['conv{}'.format(i)](samples[i]) for i in range(8)]
